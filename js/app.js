...
/* ═══════════════════════════════
   Career History 연도 탭 전환
═══════════════════════════════ */
function whSwitchYear(year) {
  // 탭 활성화
  document.querySelectorAll('.wh-year-tab').forEach(t => t.classList.remove('active'));
  const tab = document.querySelector(`.wh-year-tab[onclick*="'${year}'"]`);
  if (tab) tab.classList.add('active');

  // 블록 전환
  document.querySelectorAll('.wh-year-block').forEach(b => {
    b.classList.remove('active');
    b.style.opacity = '0';
  });
  const block = document.getElementById('wh-' + year);
  if (block) {
    block.classList.add('active');
    requestAnimationFrame(() => {
      block.style.transition = 'opacity .4s ease';
...
window.whSwitchYear = whSwitchYear;

// 초기 블록 opacity
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.wh-year-block').forEach(b => {
    b.style.opacity = b.classList.contains('active') ? '1' : '0';
  });
});
